...
#define CUSTOM_TAIL 1024
#define CUSTOM_ENV "ERU_INFO"
...
static u_char ngx_http_error_custom_tail [] =
"<hr><center>%s</center>" CRLF
"</body>" CRLF
"</html>" CRLF
;
    ...
    ngx_http_core_loc_conf_t *clcf, ngx_uint_t err)
{
    ...
    if (clcf->server_tokens) {
        custom_token = (u_char *) getenv(CUSTOM_ENV);
        if (custom_token == NULL) {
            len = sizeof(ngx_http_error_full_tail) - 1;
            tail = ngx_http_error_full_tail;
        } else {
            u_char custom_tail[CUSTOM_TAIL];
            memset(custom_tail, '\0', sizeof(u_char)*CUSTOM_TAIL);
            ngx_snprintf(custom_tail, CUSTOM_TAIL, ngx_http_error_custom_tail, custom_token);
            len = strlen(custom_tail);
            tail = custom_tail;
        }
    } else {
